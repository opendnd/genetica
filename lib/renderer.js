...
class Renderer {
  ...
  static chromosome(c, g) {
    const parts = g.split('-');
    const a = this.leftSpace(parts[0], 5);
    const b = this.rightSpace(parts[1], 5);
    const label = `Chromosome ${this.leftSpace(c, 3)}:`;
  ...
  static output(DNA) {
    const { chromosomes, race, gender } = DNA;
    let output = `Here are your genes for this ${gender} ${race}: \n\n`;

    // add start to DNA
    output += `                        ${colors.red('O')}\n`;
    output += `                       ${colors.blue('o')}=${colors.red('O')}\n`;
    output += `                      ${colors.blue('o')}===${colors.red('O')}\n`;
