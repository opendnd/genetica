    ...
    Object.keys(opts).forEach((opt) => {
      if (opt.includes('chromosome-')) {
        ...
        if (opts.gender === 'female') {
          if (!parts[0].includes('X')) throw new Error('Malformatted sex chromosome input!');
          if (!parts[1].includes('X')) throw new Error('Malformatted sex chromosome input!');
        }

        if (opts.gender === 'male') {
          if (!parts[0].includes('X')) throw new Error('Malformatted sex chromosome input!');
          if (!parts[1].includes('Y')) throw new Error('Malformatted sex chromosome input!');
        }
      }
    });
