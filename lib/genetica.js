...
class Genetica {
      ...
      if (opt.includes('chromosome-')) {
        ...
        if (opt === 'chromosome-sex') {
          if (opts.gender === 'female') {
            if (!partA.includes('X')) throw new Error('Malformatted female sex chromosome input!');
            if (!partB.includes('X')) throw new Error('Malformatted female sex chromosome input!');

            partA = parseInt(partA.replace('X', ''), 10);
            partB = parseInt(partB.replace('X', ''), 10);

            if (partA > XdiceValue) throw new Error('Sex chromosome value higher than possible dice roll!');
            if (partB > XdiceValue) throw new Error('Sex chromosome value higher than possible dice roll!');
          }
          if (opts.gender === 'male') {
            if (!partA.includes('X')) throw new Error('Malformatted male sex chromosome input!');
            if (!partB.includes('Y')) throw new Error('Malformatted male sex chromosome input!');

            partA = parseInt(partA.replace('X', ''), 10);
            partB = parseInt(partB.replace('Y', ''), 10);

            if (partA > XdiceValue) throw new Error('Sex chromosome value higher than possible dice roll!');
            if (partB > YdiceValue) throw new Error('Sex chromosome value higher than possible dice roll!');
          }
        } else {
          if (partA > diceValue) throw new Error('Chromosome value higher than possible dice roll!');
          if (partB > diceValue) throw new Error('Chromosome value higher than possible dice roll!');
        }
      }
    });
  ...
  generateChild(opts = {}, motherDNA = {}, fatherDNA = {}) {
    ...
    const { race, gender } = this.opts;
    const chromosomes = this.generateChildChromosomes(motherDNA.chromosomes, fatherDNA.chromosomes);
  ...
  generateParents(DNA) {
    ...
    Object.keys(template.chromosomes).forEach((c) => {
      const dice = template.chromosomes[c];
      const parts = chromosomes[c].split('=');
      const partA = parts[0];
      const partB = parts[1];
      let chromosomeName;
